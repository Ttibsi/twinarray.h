...
namespace boost::inline ext::ut::inline v2_1_1 {
    namespace utility {
        ...
        [[nodiscard]] inline auto is_match(std::string_view input, std::string_view pattern)
            -> bool {
...
// for over compiler need over macros
    ...
    namespace type_traits {
        ...
        template <class T>
        inline constexpr auto has_user_print =
        ...
        struct has_static_member_object_epsilon<T, std::void_t<decltype(declval<T>().epsilon)>>
            : std::bool_constant<
                  !std::is_member_pointer_v<decltype(&T::epsilon)> &&
        ...
        struct eq_ : op {
            constexpr eq_(const TLhs& lhs = {}, const TRhs& rhs = {})
                : lhs_ {lhs}, rhs_ {rhs}, value_ {[&] {
                      ...
                      if constexpr (
                          type_traits::has_static_member_object_value_v<TLhs> and
                          type_traits::has_static_member_object_value_v<TRhs>) {
                          return TLhs::value == TRhs::value;
                      } else if constexpr (
                          type_traits::has_static_member_object_epsilon_v<TLhs> and
       ...
       public:
        ...
        template <
            ...
            type_traits::requires_t<
                not type_traits::has_user_print<T> and type_traits::is_range_v<T>> = 0>
        auto& operator<<(T&& t) {
            *this << '{';
            auto first = true;
        ...
        auto& operator<<(const detail::approx_<TLhs, TRhs, TEpsilon>& op) {
            return (
                *this << color(op) << op.lhs() << " ~ (" << op.rhs() << " +/- " << op.epsilon()
                      << ')' << colors_.none);
        }
        ...
        auto& operator<<(const detail::and_<TLhs, TRhs>& op) {
            return (
                *this << '(' << op.lhs() << color(op) << " and " << colors_.none << op.rhs()
                      << ')');
        }
        ...
        auto& operator<<(const detail::or_<TLhs, TRhs>& op) {
            return (
                *this << '(' << op.lhs() << color(op) << " or " << colors_.none << op.rhs() << ')');
        }
        ...
        auto& operator<<(const detail::throws_<TExpr, TException>& op) {
            return (
                *this << color(op) << "throws<" << reflection::type_name<TException>() << ">"
                      << colors_.none);
        }
        ...
        auto on(events::summary) -> void {
            if (tests_.fail or asserts_.fail) {
                printer_ << "\n========================================================"
                         ...
                         << " passed" << " | " << printer_.colors().fail << asserts_.fail
        ...
        void print_console_summary(std::ostream& out_stream, std::ostream& err_stream) {
            for (const auto& [suite_name, suite_result] : results_) {
                if (suite_result.fails) {
                    ...
                    out_stream << color_.pass << "Suite '" << suite_name << "': all tests passed"
...
#pragma clang diagnostic ignored "-Wunused-comparison"
            ...
            template <
                class TLhs,
                class TRhs,
                type_traits::requires_t<type_traits::is_op_v<TLhs> or type_traits::is_op_v<TRhs>> =
                    0>
    ...
    template <
        class TExpr,
        type_traits::requires_t<
            type_traits::is_op_v<TExpr> or type_traits::is_convertible_v<TExpr, bool>> = 0>
